...
def capture_images():
    ...
    folder_name = os.path.join(os.path.dirname(__file__), "static", "imagenes_cam3", "dia_" + current_date)
...
def stop_capture():
    ...
    while threading.active_count() > 1:
        time.sleep(1)
    #sync_with_server()
    return 'Captura de imágenes detenida, Se enviaran automaticamente las imagenes al servidor a las 21h de manera automatica.'
...
def sync_with_server():
    ...
    local_folder = '/home/camera2/Desktop/API_FOTOS/static/imagenes_cam3'

    # Parámetros de la sincronización
    remote_folder = '/mnt/nvme/chemplate/static/imagenes_cam3'
...
def gen_frames():
    ...
    DATA_FOLDER = "/home/camera2/Desktop/API_FOTOS/tensorFlowLite/"
    model_path = DATA_FOLDER + "model3_cubeta5.tflite"
...
def save_to_database(timestamp, estacio, estat, confidence, image_base64):
    try:
        ...
        cur.execute("""
            INSERT INTO images_cubeta5 (timestamp, estacio, estat, confidence, imatge)
            VALUES (%s, %s, %s, %s, %s)
...
if __name__ == '__main__':
    app.run(debug=True, host='192.168.50.115')
